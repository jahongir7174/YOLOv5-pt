...
def labels_to_class_weights(labels, nc=80):
    ...
    # Prepend grid-point count (for uCE training)
    # gpi = ((320 / 32 * np.array([1, 2, 4])) ** 2 * 3).sum()  # grid-points per image
    # weights = np.hstack([gpi * len(labels)  - weights.sum() * 9, weights * 9]) ** 0.5  # prepend grid-points to start
